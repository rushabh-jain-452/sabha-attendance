<?php
    ...
    if(isset($_GET['date'])) {
        ...
        if(strcmp($param, "Absent") == 0) {
            $sql = "SELECT memberid, name, gender, dob, mobileno, address, '' as timestamp FROM member WHERE memberid NOT IN (SELECT memberid from attendance WHERE date = '$date') AND mandal='$mandal' AND active=true";
        }
